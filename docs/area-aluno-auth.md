...
- Proteção server-side do conteúdo: `functions/area-aluno/_middleware.ts` e `functions/area-admin/_middleware.ts` validam a sessão antes de servir qualquer página de `/area-aluno/*` e `/area-admin/*` (a tela de login fica liberada). Sem isso o gating seria só de UX e o conteúdo premium vazaria pela URL direta. O guard client-side (`auth-guard.js`) continua, mas a barreira real é o middleware.
- Conteúdo premium (cenários, ferramentas Pro, checklist, alertas, painel admin) fica **fora do repositório público** (open-core, ver `.gitignore` e `README.md`). Os arquivos seguem no disco para deploy via `wrangler` e devem ser versionados em repositório privado.
